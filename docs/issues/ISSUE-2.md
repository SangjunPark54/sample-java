...
updated_at: 2026-05-13T08:27:36.295876
resolved_at: 2026-05-13T08:21:50.088930
related_tickets:
...
## Plan (LLM-generated)
1. PostService.getPost(Long)에서 postRepository.findById(id).get()을 orElseThrow(new PostNotFoundException(id))로 변경해야 한다.
2. PostNotFoundException 클래스를 새로 생성하거나, 만약 파일 트리에 없다면 기존 파일에 내부 클래스로 추가 구현해야 한다. 하지만 트리에 별도 예외 패키지 및 파일이 없어, post_service.java 내에 추가해야 한다.
3. GlobalExceptionHandler에서 @ExceptionHandler(PostNotFoundException.class) 구현이 필요하지만, 파일 트리에 예외 핸들러(예: GlobalExceptionHandler.java)가 존재하지 않는다. 따라서 해당 처리는 Controller에서 구현해야 하며, PostController.java에서 404 반환 로직을 추가해야 한다.
