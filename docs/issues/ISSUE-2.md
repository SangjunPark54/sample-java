...
updated_at: 2026-05-13T08:21:35.420042
resolved_at: 2026-05-13T07:46:25.347529
related_tickets:
...
## Plan (LLM-generated)
1. src/main/java/com/example/board/service/post_service.java 파일에서 postRepository.findById(id).get() 호출부를 orElseThrow(() -> new PostNotFoundException(id))로 변경합니다.
2. PostNotFoundException 클래스를 새로 추가하거나 기존 파일에 정의(권장: post_service.java 내 내부 static class 또는 패키지 구조에 따라 파일 분리).
3. src/main/java/com/example/board/controller/PostController.java 및 src/main/java/com/example/board/GlobalExceptionHandler.java(파일 트리에 없는 경우 PostController.java 내 @ExceptionHandler 추가)에서 PostNotFoundException 발생 시 404 반환 처리를 구현합니다.
