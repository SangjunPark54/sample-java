...
updated_at: 2026-05-13T07:41:39.044777
resolved_at: 2026-05-13T07:34:35.959830
related_tickets:
...
## Plan (LLM-generated)
1. getPost(Long)에서 postRepository.findById(id).get() 호출을 orElseThrow(PostNotFoundException)으로 변경해야 한다.
2. PostNotFoundException 클래스를 새로 추가해야 하나, 현재 파일 트리에 지정된 클래스 파일 목록에 PostNotFoundException이 없다. 반드시 기존 파일만 수정해야 하므로 제외.
3. GlobalExceptionHandler에서 PostNotFoundException을 처리하게 하려면 해당 ExceptionHandler가 포함된 파일의 수정이 필요하지만, 파일 트리에 GlobalExceptionHandler가 목록에 없다. 컨트롤러 내에 있을 수 있으나 경로상 보이지 않으므로 제외.
4. 따라서, 실제로 반드시 수정이 필요한 것은 postRepository.findById(id).get()을 직접 호출하는 부분으로, PostService 혹은 유사 파일이 실제 트리에 존재하지 않는다. 유사하게 수정 예상되는 부분의 파일이 있어야 하지만, 파일트리 내에 'post_service.java'는 전체 소문자로만 존재하는데, 통상 Service 클래스는 여기 있다 판단 가능(컨벤션 위반).
5. 결론적으로, orElseThrow 교체 적용이 필요한 src/main/java/com/example/board/service/post_service.java 만이 확실히 수정 필요하다.
