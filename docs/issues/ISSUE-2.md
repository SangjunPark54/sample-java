...
updated_at: 2026-05-13T07:34:20.657632
resolved_at: 2026-05-13T07:27:53.065113
related_tickets:
...
## Plan (LLM-generated)
1. NullPointerException의 직접 원인인 postRepository.findById(id).get() 호출을 post_service.java에서 orElseThrow(PostNotFoundException)으로 변경한다.
2. PostNotFoundException을 service 디렉토리에 추가(단, 파일 신규 생성은 file_tree에 없으므로 제외).
3. GlobalExceptionHandler의 PostNotFoundException 매핑(@ExceptionHandler)을 추가하여 404 응답을 반환한다.
4. 위 핸들러는 일반적으로 PostController.java에 포함될 가능성이 높으므로 이 파일도 수정한다.
5. repository/PostRepository.java는 findById만 제공하며 로직 변경 필요 없음.
