...
updated_at: 2026-05-14T07:30:17.576544
resolved_at: 2026-05-14T07:04:18.600876
related_tickets:
...
## Plan (LLM-generated)
1. PostService.getPost(Long) 메서드에서 postRepository.findById(id).get() 사용 부분을 orElseThrow(() -> new PostNotFoundException(id))로 수정해야 한다.
2. PostNotFoundException 클래스를 추가하거나(파일이 없으므로 여기에 추가할 수 없음) 별도 파일 생성 필요하지만, file_tree에 없음.
3. GlobalExceptionHandler에서 PostNotFoundException을 404로 매핑해야 하나 해당 파일 트리에 Handler 클래스가 없음.
따라서, 현재 트리 내에서는 getPost()를 가진 파일만 수정이 필요.
