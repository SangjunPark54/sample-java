...
updated_at: 2026-05-14T07:04:01.419587
resolved_at: 2026-05-13T09:15:35.692838
related_tickets:
...
## Plan (LLM-generated)
1. PostService.getPost(Long)에서 findById(id).get()를 orElseThrow(() -> new PostNotFoundException(id))로 수정한다.
2. PostNotFoundException 클래스를 생성하고 RuntimeException을 상속, HttpStatus.NOT_FOUND에 매핑된다.
3. GlobalExceptionHandler에서 PostNotFoundException을 404로 매핑하는 @ExceptionHandler를 추가한다.

수정이 반드시 필요한 파일은 post_service 관련 코드(비정규 표기지만), 예외 클래스(PostNotFoundException)와 글로벌 핸들러가 포함되어야 하나, 이 프로젝트의 파일트리에는 GlobalExceptionHandler나 별도 예외 클래스 파일이 없다. 그러므로 post_service.java 파일만 대상. 예외, 핸들러 추가는 새로운 파일이 추가적으로 필요하나 트리에 없어 plan에만 명시.
