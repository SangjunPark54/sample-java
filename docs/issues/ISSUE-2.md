...
updated_at: 2026-05-14T08:06:50.784310
resolved_at: 2026-05-14T07:30:36.694005
related_tickets:
...
## Plan (LLM-generated)
1. PostService.getPost(Long)에서 postRepository.findById(id).get()을 orElseThrow(() -> new PostNotFoundException(id))으로 교체합니다.
2. PostNotFoundException 클래스를 생성하거나, 이미 있다면 수정합니다(단, 파일트리에 없음 → 생성 필요). 
3. GlobalExceptionHandler에 @ExceptionHandler(PostNotFoundException.class) 및 404 반환 로직을 추가합니다(동일 파일에 있을 수 있음). 
4. 제공된 파일트리에서 service/post_service.java에 getPost 관련 로직이 있을 것으로 판단되며, ExceptionHandler(location) 또는 Exception 관련 처리는 controller 또는 service 단에 존재할 가능성이 높음.
5. 파일트리에 PostService 또는 ExceptionHandler 전용 파일, PostNotFoundException 정의 파일은 없음. 따라서 반드시 수정이 필요한 실제 소스파일은 post_service.java 하나입니다.
