...
updated_at: 2026-05-13T09:15:15.793812
resolved_at: 2026-05-13T09:04:40.819624
related_tickets:
...
## Plan (LLM-generated)
1. postRepository.findById(id).get() 호출을 orElseThrow(() -> new PostNotFoundException(id))로 대체해야 합니다.
2. PostNotFoundException 클래스를 생성해야 합니다. (단, 파일트리에 존재하지 않으므로 이번 선택에서는 제외)
3. GlobalExceptionHandler에 PostNotFoundException 핸들러 추가가 필요하나, 해당 파일이 트리에 없으므로 수정 불가.
4. 즉, getPost(Long) 메서드가 위치한 구현 파일만 수정이 필요합니다.
